...
from .models import Writer, Book, Review, Slider, Category, Course
...
def index(request):
    ...
    context = {
        ...
        "slide": slide,
        "category": category,
        "course": course # href="{% url 'store:category' id=p.id %}"
    }
...
def get_book_category(request, id):
    ...
    category = Category.objects.all()
    course = Course.objects.all()
    return render(request, "store/category.html", {'book': book, 'category': category, 'course': course})
